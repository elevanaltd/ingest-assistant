...
export class SecurityValidator {
   ...
   * Resolves symlinks for consistency with validateFilePath().
   */
  async setAllowedBasePath(folderPath: string): Promise<void> {
    ...
    console.log(`[SecurityValidator] Base path set to: ${this.allowedBasePath}`);
  }

  /**
   * Add additional allowed path (e.g., transcode cache directory).
   * Used for temporary files that are generated by the app.
   * Resolves symlinks to canonical paths for macOS compatibility.
   */
  async addAllowedPath(allowedPath: string): Promise<void> {
    // Resolve symlinks (e.g., macOS /var -> /private/var)
    const resolved = await fs.realpath(allowedPath).catch((error) => {
      // Log symlink resolution failures for debugging
      console.warn(`[SecurityValidator] Failed to resolve symlink for ${allowedPath}:`, error.message);
      return path.resolve(allowedPath);
    });

    if (!this.additionalAllowedPaths.includes(resolved)) {
      this.additionalAllowedPaths.push(resolved);
      console.log(`[SecurityValidator] Added allowed path: ${resolved}`);
    }
  }
  ...
  async validateFilePath(filePath: string): Promise<string> {
    ...
    // Use path.resolve() as fallback to avoid realpath() errors on non-existent files
    const resolvedPath = await fs.realpath(filePath).catch(() => path.resolve(filePath));

    // Check against primary allowed base path
    const relativePath = path.relative(this.allowedBasePath, resolvedPath);
    const isInBasePath = !relativePath.startsWith('..') && !path.isAbsolute(relativePath);

    if (isInBasePath) {
      return resolvedPath;
    }

    // Check against additional allowed paths (e.g., transcode cache)
    for (const allowedPath of this.additionalAllowedPaths) {
      const relativeToAllowed = path.relative(allowedPath, resolvedPath);
      const isInAllowedPath = !relativeToAllowed.startsWith('..') && !path.isAbsolute(relativeToAllowed);
    ...
    // Not in any allowed path - reject
    throw new SecurityViolationError(
      'PATH_TRAVERSAL',
      filePath,
      `Access denied: Path outside allowed folders (base: ${this.allowedBasePath}, additional: ${this.additionalAllowedPaths.join(', ')})`
    );
  }
